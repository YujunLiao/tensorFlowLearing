...
def savetxt(fname, X, fmt='%.18e', delimiter=' ', newline='\n', header='',
    ...
    >>> np.savetxt('code.out', x, delimiter=',')   # X is an array
    >>> np.savetxt('code.out', (x,y,z))   # x,y,z equal sized 1D arrays
    >>> np.savetxt('code.out', x, fmt='%1.4e')   # use exponential notation
...
def fromregex(file, regexp, dtype, encoding=None):
    ...
    >>> output = np.fromregex('code.dat', regexp,
    ...                       [('num', np.int64), ('key', 'S3')])
    >>> output
    ...
    * Individual values are not stripped of spaces by default.
    ...
    for (j, conv) in user_converters.items():
        ...
        if _is_string_like(j):
            ...
            i = j
        # Find the value to code - first_line is not filtered by usecols:
        if len(first_line):
