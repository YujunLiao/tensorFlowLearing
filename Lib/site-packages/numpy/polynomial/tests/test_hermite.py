...
class TestIntegral(object):
    ...
    def test_hermint(self):
        ...
        # code integration of zero polynomial
        for i in range(2, 5):
            k = [0]*(i - 2) + [1]
...
class TestFitting(object):
    ...
    def test_hermfit(self):
        ...
        assert_almost_equal(wcoef2d, np.array([coef3, coef3]).T)
        # code scaling with complex values x points whose square
        # is zero when summed.
        x = [1, 1j, -1, -1j]
        assert_almost_equal(herm.hermfit(x, x, 1), [0, .5])
        assert_almost_equal(herm.hermfit(x, x, [0, 1]), [0, .5])
        # code fitting only even Legendre polynomials
        x = np.linspace(-1, 1)
        y = f2(x)
...
class TestGauss(object):
    ...
    def test_100(self):
        ...
        # code orthogonality. Note that the results need to be normalized,
