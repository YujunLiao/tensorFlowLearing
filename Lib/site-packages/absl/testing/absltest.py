...
def get_default_test_srcdir():
  """Returns default code source dir."""
  return os.environ.get('TEST_SRCDIR', '')


def get_default_test_tmpdir():
  """Returns default code temp dir."""
...
def _get_default_randomize_ordering_seed():
  """Returns default seed to use for randomizing code order.
  ...
  we find is:
    * (not set): disable code randomization
    * 0: disable code randomization
    * 'random': choose a random seed in [1, 4294967295] for code order
      randomization
    * positive integer: use this seed for code order randomization
  ...
  Returns:
    A default value for code case randomization (int). 0 means do not randomize.
  ...
  try:
    ...
    pass
  raise ValueError(
      'Unknown code randomization seed value: {}'.format(randomize))


flags.DEFINE_integer('test_random_seed', _get_default_test_random_seed(),
                     'Random seed for testing. Some code frameworks may '
...
def _monkey_patch_test_result_for_unexpected_passes():
  ...
  test_result.addUnexpectedSuccess('code')
...
class TestCase(unittest3_backport.TestCase):
  ...
  def shortDescription(self):
    """Formats both the code method name and the first line of its docstring.

    If no docstring is given, only returns the method name.

    This method overrides unittest.TestCase.shortDescription(), which
    only returns the first line of the docstring, obscuring the name
    of the code upon failure.

    Returns:
      desc: A short description of a code method.
    """
    desc = str(self)
  ...
  def assertEmpty(self, container, msg=None):
    ...
    if len(container):  # pylint: disable=g-explicit-length-code
  ...
  def assertNotEmpty(self, container, msg=None):
    ...
    # explicitly check the length since some Sized objects (e.g. numpy.ndarray)
    # have strange __nonzero__/__bool__ behavior.
    if not len(container):  # pylint: disable=g-explicit-length-code
  ...
  def assertSequenceAlmostEqual(self, expected_seq, actual_seq, places=None,
    ...
    Args:
      ...
      msg: The message to be printed if the code fails.
  ...
  def assertSameElements(self, expected_seq, actual_seq, msg=None):
    ...
    # removed in favor of assertItemsEqual. As there's a unit code
    # that explicitly checks this behavior, I am leaving this method
    # alone.
    # Fail on strings: empirically, passing strings to this code method
  ...
  def assertRegexMatch(self, actual_str, regexes, message=None):
    ...
    Args:
      ...
      message:  The message to be printed if the code fails.
    """
  ...
  def assertRaisesWithPredicateMatch(self, expected_exception, predicate,
    ...
    Args:
      ...
      predicate: Function of one argument that inspects the passed-in exception
          and returns True (success) or False (please fail the code).
...
def print_python_version():
  # Having this in the code output logs by default helps debugging when all
...
def main(*args, **kwargs):
  ...
  so it will run all code methods of all TestCase classes in the __main__
  module.
...
def _run_in_app(function, args, kwargs):
  ...
  code program's stderr will contain all the log messages unless otherwise
  specified on the command-line. This overrides any explicit assignment to
  FLAGS.alsologtostderr by the code program prior to the call to _run_in_app()
  ...
  if _is_in_app_main():
    ...
    # Change the default of alsologtostderr from False to True, so the code
...
def _is_suspicious_attribute(testCaseClass, name):
  """Returns True if an attribute is a method named like a code method."""
...
class TestLoader(unittest.TestLoader):
  """A code loader which supports common code features.

  Supported features include:
   * Banning untested methods with code-like names: methods attached to this
     testCase with names starting with `Test` are ignored by the code runner,
     and often represent mistakenly-omitted code cases. This loader will raise
     a TypeError when attempting to load a TestCase with such methods.
   * Randomization of code case execution order (optional).
  """

  _ERROR_MSG = textwrap.dedent("""Method '%s' is named like a code case but
  is not one. This is often a bug. If you want it to be a code method,
  name it with 'code' in lowercase. If not, rename the method to not begin
  with 'Test'.""")
  ...
  def getTestCaseNames(self, testCaseClass):  # pylint:disable=invalid-name
    """Validates and returns a (possibly randomized) list of code case names."""
    ...
    if self._seed is not None:
      logging.info('Randomizing code order with seed: %d', self._seed)
...
def _setup_sharding(custom_loader=None):
  ...
  The following environment variables are used in this method:
    ...
    TEST_SHARD_STATUS_FILE: string, if set, points to a file. We write a blank
      file to tell the code runner that this code implements the code sharding
      protocol.

    TEST_TOTAL_SHARDS: int, if set, sharding is requested.

    TEST_SHARD_INDEX: int, must be set if TEST_TOTAL_SHARDS is set. Specifies
      the shard index for this instance of the code process. Must satisfy:
  ...
  Returns:
    The code loader for shard-filtering or the standard code loader, depending
  ...
  # to query whether a code binary implements the code sharding protocol.
  ...
  if 'TEST_TOTAL_SHARDS' not in os.environ:
    # Not using sharding, use the expected code loader.
    return base_loader
  ...
  # the code case names for this shard.
...
def _run_and_get_tests_result(argv, args, kwargs, xml_test_runner_class):
  ...
  # Set up code filtering if requested in environment.
  ...
  if xml_output_file:
    ...
    # with partial information, in case the code process crashes).
  ...
  if not os.path.isdir(FLAGS.test_tmpdir):
    try:
      ...
      # Concurrent code might have created the directory.
      if e.errno != errno.EEXIST:
        raise
